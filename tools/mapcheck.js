...
(function () {
  ...
  /* Потолок поднят с 700 до 900 вместе с разведением палитры по породам.
     Считаются меши ПОСЛЕ склейки, а склейка идёт по материалу x ячейке 60 м:
     каждый новый материал стоит примерно три меша на зал. Кирпич, гофролист,
     ржавый лист, бетон и прокат — это и есть цена ответа на «всё дерево на
     одно лицо». Настоящий бюджет тут треугольники (82 тыс. из 220 тыс.) и
     вызовы отрисовки, а их замер по восьми ракурсам держится в пределах
     сотен. */
  if (meshes > 900) bad('§7.6 мешей больше 900', meshes);
