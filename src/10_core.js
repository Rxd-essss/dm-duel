...
/* Версия одной строкой: подставляется в заголовок, паузу, лобби и экран итогов.
   Раньше «v3» был рассыпан литералами по шести файлам, и разгон версии
   превращался в поиск по строкам. */
const VER = 'v4';
...
const CFG = {
  ...
  /* --- канонические высоты этажей (MAPDESIGN §1) ---
     Прошлая карта читалась как набор объектов именно потому, что площадки
     стояли на случайных высотах (1.4, 5.8, 10.2, 14.8, 16.2). Игрок не мог
     сказать, на каком он этаже. Теперь пол любой площадки обязан лежать на
     одном из этих уровней, а всё, что вне допуска, — ошибка планировки. */
  /* Четыре яруса деревянного зала: земля, две галереи и стропила под крышей.
     Шаг 6 м выбран так, чтобы с яруса было видно соседние, но прыжок вниз
     без урона не проходил, и чтобы человек на доске читался снизу силуэтом. */
  floor1: 6.0,
  floor2: 12.0,
  floor3: 18.0,
  floorTol: 0.8
...
/* ====================== ОРУЖИЕ И БОЕПРИПАС ======================
   ...
   Стволов два, и они играются по-разному, а не отличаются моделью.

   ВИНТОВКА — плоская траектория, оптика, затвор. Ошибка стоит времени
   на перезарядку, но пуля приходит почти мгновенно.

   ЛУК — стрела летит втрое-впятеро медленнее и падает вдвое сильнее.
   На 100 м это шесть метров просадки: цель надо не «навести», а
   ОБСТРЕЛЯТЬ по дуге с упреждением. Оптики нет, зато натяг решает всё:
   недотянутый лук бьёт слабо, медленно и вразброс.

   Оба ствола держат по три типа боеприпаса, и роли типов одинаковые:
   обычный (единственный с полным критом), фугасный (площадь, крита нет)
   и зажигательный (горение и очаг, длинный откат). Так игрок переносит
   привычку с одного оружия на другое, а меняется только характер полёта.

   Поле cd — персональный откат типа: тикает всегда, даже если выбран
   другой тип. */
...
/* СТРЕЛЫ. Скорость дана при ПОЛНОМ натяге — при слабом оружейный модуль
   опускает её вместе с уроном. gMul выше единицы намеренно: дуга и есть
   главное отличие лука. bolt здесь — не затвор, а время наложить стрелу. */
const BOW_AMMO = [
  { id:'arrow', name:'СТРЕЛА', short:'СТРЕЛА', col:0xd8c89a, trail:0xf1e4bd,
    v:112, drag:0.055, gMul:1.15, windMul:1.60,
    dmgMin:26, dmgMax:88, mag:8, res:40, resMax:40, reload:2.0, bolt:0.42,
    spread:2.2, crit:true, cd:0, arrow:true,
    stat:'V0 112 м/с · урон 26→88 · крит в голову · колчан 8 · наложить 0.42 с' },

  { id:'bomb', name:'ВЗРЫВНАЯ СТРЕЛА', short:'ВЗРЫВ', col:0xe08a4a, trail:0xffb072,
    v:88, drag:0.070, gMul:1.45, windMul:1.85,
    dmgMin:16, dmgMax:42, mag:4, res:16, resMax:16, reload:2.9, bolt:0.62,
    spread:3.0, crit:false, cd:2.5, arrow:true,
    splashR:5.0, splashMax:56, splashFall:1.5, splashCover:0.45,
    stat:'V0 88 м/с · прямой 16→42 · взрыв до 56 в R 5 м · без крита · откат 2.5 с' },

  { id:'flame', name:'ОГНЕННАЯ СТРЕЛА', short:'ОГОНЬ', col:0xe2593f, trail:0xff8b4a,
    v:100, drag:0.062, gMul:1.28, windMul:1.70,
    dmgMin:15, dmgMax:36, mag:5, res:24, resMax:24, reload:2.5, bolt:0.52,
    spread:2.6, crit:true, cd:5.0, arrow:true,
    burnDps:11, burnTime:6, poolR:3.4, poolTime:8, poolDps:14,
    stat:'V0 100 м/с · прямой 15→36 + горение 11/с × 6 с · очаг огня 8 с · откат 5 с' }
];

/* Два ствола. drawTime — сколько тянуть лук до полного натяга; у винтовки
   вместо него работает прежний CFG.chargeMax при выцеливании в оптику. */
const WPNS = [
  { id:'rifle', name:'СНАЙПЕРСКАЯ ВИНТОВКА', short:'ВИНТОВКА', bow:false,
    ammo:RIFLE_AMMO, scope:true, drawTime:0,
    hint:'Плоская траектория, оптика с кратностью, крит в голову. Ошибся — плати затвором.' },
  { id:'bow',   name:'БОЕВОЙ ЛУК',           short:'ЛУК',      bow:true,
    ammo:BOW_AMMO,  scope:false, drawTime:1.05,
    /* Полный натяг обязан бить ТОЧНО В МАРКУ. Разброс — цена недотяга, а не
       постоянный налог: иначе на дистанциях этой карты лук перестаёт быть
       оружием и превращается в лотерею. Ниже drawTrue разброс растёт быстро. */
    drawTrue: 0.97,     // с этого натяга разброс равен нулю
    aimZoom: 0.30,      // ПКМ: приближение на 30% (не оптика — просто ближе)
    aimSteady: 0.55,    // и во столько же раз меньше уводит марку
    /* Средняя кнопка мыши — залп тремя стрелами выбранного типа.
       Веер нужен самому залпу, это не промах; откат площадных типов втрое
       длиннее, иначе тремя взрывными закрывался бы весь ярус разом. */
    volley: { n:3, fan:1.7, cdMul:3, drawMul:1.35 },
    hint:'Стрела летит по дуге: на 100 м просадка около пяти метров. Полный натяг бьёт точно в марку. ПКМ — прицел и дуга, СКМ — залп тремя стрелами.' }
];

/* РАБОЧИЙ БОЕКОМПЛЕКТ. Массив тот же самый на всю игру — его содержимое
   подменяется на месте при смене оружия. Так все полсотни мест, которые
   читают AMMO[i], продолжают работать без единой правки, и ни одна ссылка
   на элемент не протухает между матчами. */
const AMMO = RIFLE_AMMO.slice();

function setWeapon(i){
  const w = WPNS[(i|0) === 1 ? 1 : 0];
  AMMO.length = 0;
  for(let k=0;k<w.ammo.length;k++) AMMO.push(w.ammo[k]);
  return w;
}
function weaponOf(i){ return WPNS[(i|0) === 1 ? 1 : 0]; }
