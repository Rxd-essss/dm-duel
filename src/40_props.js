...
function PROP_mt(k){
  ...
  switch(k){
    ...
    /* ---- НЕ ДЕРЕВО. Добавлено по жалобе «всё дерево на одно лицо».
       Зал был выложен ровно тремя породами (доска, брус, лист) в одной
       гамме. Эти шесть материалов существуют затем, чтобы в кадре рядом с
       доской всегда стояло что-то принципиально другое по фактуре: обожжённый
       кирпич, гофрированный лист, ржавчина, бетон, прокатная сталь. */
    case 'brick':   m = toonT(0x6f6a62,'brick',3,2); break;    // кладка цоколя
    case 'brickD':  m = toonT(0x45423d,'brick',3,2); break;    // закопчённая, у печи
    case 'brickL':  m = toonT(0x837c71,'brick',3,2); break;    // выгоревшая на свету
    case 'corrug':  m = toonT(0x5f676c,'metal',10,2); break;   // гофролист обшивки
    case 'corrugR': m = toonT(0x6f452f,'rust',7,2); break;     // он же ржавый
    case 'concD':   m = toonT(0x4d4a45,'conc',3,3); break;     // бетонные основания
    case 'steel':   m = toonT(0x3f454c,'plate',2,2); break;    // прокат ферм и связей
