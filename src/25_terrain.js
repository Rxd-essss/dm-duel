  ...
  for(let iy=0; iy<ST; iy++){
    for(let ix=0; ix<ST; ix++){
      ...
      if(wall < TER_HAP){
        ...
        cr = lerp(cr, 0.46*lv, inH);
        cg = lerp(cg, 0.30*lv, inH);
        cb = lerp(cb, 0.17*lv, inH);
...
function TER_makeSkyDome(){
  const mat = new THREE.ShaderMaterial({
    uniforms:{
      cTop :{value:new THREE.Color(0x3878c2)},
      cMid :{value:new THREE.Color(0x8fc0e4)},
      cHaze:{value:new THREE.Color(0xd2e2ec)},
      cBot :{value:new THREE.Color(0xe6dcc4)},
...
const TER_SHAFT_LEN = 24.0;   // длина вдоль луча; до пола он не доживает
...
function TER_makeShafts(){
  ...
  /* ПОДКРОВЕЛЬНАЯ ДЫМКА. Одних столбов мало: замер показал, что ярус 3 всё
     равно остаётся самым тёмным местом зала — наверху в кадре стоит изнанка
     кровли, а её не освещает ничто. Поэтому у каждого проёма к столбу
     добавляется короткий широкий «нимб»: тот же свет, но разлитый в воздухе
     под самой крышей. Он и лечит порядок яркостей по ярусам, и физически
     честен — пыль у проёма светится сильнее всего. Широкое и слабое вместо
     узкого и горячего: так же поднимает верх, но не выбивает пиксели в белое. */
  const HALO_W = 5.0;    // во сколько раз нимб шире столба
  const HALO_L = 10.0;   // его длина вдоль луча
  const HALO_A = 0.22;   // и во сколько раз он слабее

  /* Склейка своя: вместе с позицией и нормалью надо перенести положение
     вершины ВДОЛЬ луча и силу столба — по ним шейдер строит профиль. */
  const parts = [];
  let total = 0;
  const cone = (cx, cz, r, L, amp) => {
    // openEnded: торцы не нужны, они бы дали жёсткие круги на срезах
    const g = new THREE.CylinderGeometry(r, r*1.45, L, 10, 1, true).toNonIndexed();
    const pa = g.attributes.position.array, n = g.attributes.position.count;
    const bm = new Float32Array(n*2);
    for(let k=0;k<n;k++){
      bm[k*2  ] = (L*0.5 - pa[k*3+1])/L;   // 0 у проёма, 1 в конце луча
      bm[k*2+1] = amp;
    }
  ...
  for(let i=0;i<S.length;i++){
    ...
    cone(S[i][0], S[i][1], r,          TER_SHAFT_LEN, a);
    cone(S[i][0], S[i][1], r*HALO_W,   HALO_L,        a*HALO_A);
  }
  const PA = new Float32Array(total*3), NA = new Float32Array(total*3), BM = new Float32Array(total*2);
  let o = 0;
  ...
  geo.setAttribute('aBeam',    new THREE.BufferAttribute(BM,2));

  const mat = new THREE.ShaderMaterial({
    uniforms:{
      uT:{value:0},
      /* Сила столбов. Подобрана замером и упирается в потолок приёмки по
         светам: почти ВСЕ пиксели ярче 235 в кадре дают именно лучи (без них
         доля падает с 3.8% до 0.01%), поэтому их яркость и есть тот рычаг,
         которым эта доля держится в рамке 0.5…4%. 0.78 даёт 2.9% — с запасом
         в обе стороны, чтобы кадр у самой кровли не выбивало. */
      uI:{value:0.68},
      cWarm:{value:new THREE.Color(0xffeec6)},
      cCold:{value:new THREE.Color(PAL.arcane)}
    },
    vertexShader: [
      'attribute vec2 aBeam;',
      'varying vec2 vB; varying vec3 vW; varying vec3 vN; varying vec3 vV;',
      'void main(){',
    ...
    fragmentShader: [
      'varying vec2 vB; varying vec3 vW; varying vec3 vN; varying vec3 vV;',
      'uniform float uT; uniform float uI; uniform vec3 cWarm; uniform vec3 cCold;',
      'void main(){',
      // толщина луча на просвет: в середине силуэта максимум, к краям ноль
      '  float e = 1.0 - abs(dot(normalize(vN), normalize(vV)));',
      '  float core = e*e;',
      // вдоль: полная сила у кровли, к полу сходит на нет
      '  float len = smoothstep(0.0, 0.05, vB.x)*(1.0 - smoothstep(0.38, 1.0, vB.x));',
      ...
      '  float dust = 0.80 + 0.28*g;',
      // эфирная искра: редкая холодная вспышка в тёплом луче
      '  float sp = max(g, 0.0); sp *= sp; sp *= sp; sp *= sp*sp;',
      '  vec3 c = cWarm*dust + cCold*sp*0.85;',
      '  gl_FragColor = vec4(c, core*len*uI*vB.y);',
      '}'
    ].join('\n'),
