...
function TER_makeShafts(){
  ...
  /* ПЕРЕСМОТР ПО ЖАЛОБЕ ЗАКАЗЧИКА: «всё пересвечено и какие-то стрёмные лучи
     из ниоткуда падают вниз». Обе беды — от одного: столбов было слишком
     много и каждый нёс ещё нимб вдвое шире себя. Аддитивная геометрия
     складывается сама с собой, поэтому кадр, где в поле зрения попадали
     три-четыре столба с нимбами, забеливался целиком, а сами лучи читались
     не как свет из проёма, а как повисшие в воздухе конусы.
     Нимб урезан вдвое по ширине и вчетверо по силе: он должен смягчать
     кромку луча, а не быть вторым лучом. */
  const HALO_W = 1.45;    // во сколько раз нимб шире столба
  const HALO_L = 5.0;     // его длина вдоль луча
  const HALO_A = 0.11;    // и во сколько раз он слабее
  ...
  /* Столбов стало вдвое меньше: берём каждый второй из середины пролёта и
     все пристенные (они слабее и жмутся к стене). Свет из проёма — акцент,
     который отмечает место, а не общее освещение зала: освещать обязаны
     лампы и непрямой, они и делают это предсказуемо на любом ярусе. */
  const TER_BEAM_K = 0.16;   // общий множитель силы столбов
  for(let i=0;i<S.length;i++){
    const wall = S[i][4] !== undefined;
    if(!wall && (i & 1)) continue;          // проредить срединные
    const r = S[i][2], a = S[i][3]*TER_BEAM_K;
