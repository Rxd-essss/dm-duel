...
const TX_GEN = {
  ...
  /* Доски вдоль U: настилы, мостики, стены сараев.
     ------------------------------------------------------------------
     ПЕРЕРИСОВАНО ПО ЖАЛОБЕ «текстура дерева однообразная и некачественная».
     Причин однообразия было три, и все три лечатся здесь.

     1. ОДИН ОТТЕНОК НА ВСЮ ПЛИТКУ. Доски различались только светлотой
        (v = 0.86…1.02), а тон у всех был один — тот, что задан tint'ом карты.
        Настоящий настил так не выглядит: доски пилены из разных брёвен, часть
        выгорела, часть намокла, и разброс у них именно ПО ЦВЕТУ. Теперь у
        каждой доски своя кисть со сдвигом тона до ±7% по каналам: одни уходят
        в серо-холодное, другие в красноватое. На кадр это влияет сильнее
        любой другой правки здесь — ровное поле распадается на отдельные доски.
     2. ЧЕТЫРЕ ДОСКИ НА ПЛИТКУ ПРИ ПОВТОРЕ 3×8. Рисунок возвращался каждые
        полметра. Досок стало пять, и ширина у них РАЗНАЯ (обычный пиленый
        лес не калиброван) — глаз перестаёт ловить период.
     3. НЕ ХВАТАЛО ТОГО, ЧТО ДЕЛАЕТ ДОСКУ ДОСКОЙ. Добавлены гвозди у торцов,
        следы пилы поперёк волокна, вытертость по центру доски и тёмная
        подтёчная кромка у шва. Всё это КРУПНАЯ деталь: правило §3.2 про
        «на 100 м текстура обязана собраться в ровное пятно» не нарушено,
        мелкого контрастного шума не добавлено ни одного. */
  plank(g,S,seed){
    const base = TX_ink(1.00, 0.955, 0.885);
    TX_srand(seed || 1701);
    g.fillStyle = base(0.94); g.fillRect(0,0,S,S);
    /* Ширины досок: пять неравных долей, в сумме ровно единица — иначе
       последняя доска не сойдётся с первой и шов пойдёт по всей карте. */
    const W = [0.225, 0.17, 0.215, 0.185, 0.205], rows = W.length;
    const u = S/256;                       // всё, что в пикселях, — от базовых 256
    let y0 = 0;
    for(let r=0;r<rows;r++){
      const hh = S*W[r];
      const v = TX_rr(0.84, 1.03);
      /* Своя кисть на доску: тон гуляет по каналам врозь. Красный вверх и
         синий вниз — доска потеплела; наоборот — выгорела на солнце.
         Разброс НАМЕРЕННО крошечный (±0.8%): дальше по конвейеру стоит
         TX_expose с показателем 3.6, и он растягивает любую разницу почти
         вчетверо. Проба с ±6% дала на стене розовые доски вперемешку с
         зелёными, ±2.5% — ту же полосатость послабее; и то и другое на
         скриншоте читалось как брак печати, а не как лес. Разброс между
         досками держит ЯРКОСТЬ (v ниже), она такой беды не создаёт. */
      const wr = TX_rr(0.992, 1.008), wb = TX_rr(0.992, 1.008);
      const ink = TX_ink(1.00*wr, 0.955*TX_rr(0.997,1.003), 0.885*wb);
      g.fillStyle = ink(v); g.fillRect(0, y0, S, hh);
      // вытертость по середине доски: по ней ходят, и она светлее кромок
      const grd = g.createLinearGradient(0, y0, 0, y0+hh);
      grd.addColorStop(0.00, ink(v*0.90, 0.55));
      grd.addColorStop(0.42, ink(v*1.07, 0.30));
      grd.addColorStop(1.00, ink(v*0.88, 0.55));
      g.fillStyle = grd; g.fillRect(0, y0, S, hh);
      // волокно — синус с ЦЕЛЫМ числом периодов на плитку, иначе на стыке излом
      const k = TX_ri(1,3), NG = Math.round(13*hh/(S*0.2));
      for(let i=0;i<NG;i++){
        const gy = y0 + TX_rr(3*u, hh-3*u), amp = TX_rr(1.0,3.2)*u, ph = TX_rr(0,6.283185);
        g.strokeStyle = ink(v*TX_rr(0.74,0.94), TX_rr(0.22,0.52));
        g.lineWidth = TX_rr(0.9,2.4)*u; g.lineCap = 'round';
        g.beginPath();
        for(let x=0;x<=S;x+=8*u){
          const yy = gy + Math.sin(x/S*6.283185*k + ph)*amp;
          if(x===0) g.moveTo(x,yy); else g.lineTo(x,yy);
        }
        g.stroke();
      }
      // следы пилорамы: слабая поперечная гребёнка, видна только вблизи
      g.strokeStyle = ink(v*0.86, 0.13); g.lineWidth = 1.1*u;
      for(let x = TX_rr(0,14*u); x < S; x += TX_rr(9,17)*u){
        g.beginPath(); g.moveTo(x, y0+2*u); g.lineTo(x + TX_rr(-3,3)*u, y0+hh-2*u); g.stroke();
      }
      if(TX_r() < 0.85)
        TX_knot(g,S,ink, TX_rr(12*u,S-12*u), y0 + hh*0.5 + TX_rr(-hh*0.2,hh*0.2), TX_rr(4.5,8)*u, v);
      // торцевой стык двух досок в ряду
      if(TX_r() < 0.55){
        const jx = TX_rr(20*u, S-20*u);
        TX_wrap(S, jx, y0+hh*0.5, 4*u, (px,py)=>{
          g.fillStyle = ink(0.50,0.8); g.fillRect(px-1.3*u, py-hh*0.5+3*u, 2.6*u, hh-6*u);
          g.fillStyle = ink(1.08,0.4); g.fillRect(px+1.3*u, py-hh*0.5+3*u, 1.2*u, hh-6*u);
        });
      }
      // гвозди: пара у каждого торца, чуть утоплены — тень сверху, блик снизу
      for(const nx of [TX_rr(7,16)*u, S - TX_rr(7,16)*u]){
        for(const ny of [y0 + hh*0.27, y0 + hh*0.73]){
          if(TX_r() < 0.3) continue;
          TX_wrap(S, nx, ny, 3.4*u, (px,py)=>{
            g.fillStyle = ink(0.44,0.75); TX_blob(g,px,py,2.0*u,0.35,7); g.fill();
            g.fillStyle = ink(1.14,0.5);  TX_blob(g,px,py-0.9*u,1.1*u,0.3,6); g.fill();
          });
        }
      }
      // шов между досками: тень сверху, светлая фаска под ней, тень снизу
      g.fillStyle = ink(0.36,0.88); g.fillRect(0, y0, S, 2.4*u);
      g.fillStyle = ink(1.08,0.45); g.fillRect(0, y0+2.4*u, S, 1.6*u);
      g.fillStyle = ink(0.58,0.50); g.fillRect(0, y0+hh-3.2*u, S, 3.2*u);
      y0 += hh;
    }
    TX_speck(g,S,base,90,0.5*u,1.6*u,0.62,0.9,0.3);
    /* КРУПНЫХ ПЯТЕН ЗДЕСЬ НЕТ, И ЭТО РЕШЕНИЕ, А НЕ ПРОПУСК.
       Первая проба добавляла поверх доски мягкие пятна сырости и выгорания
       через multiply/screen — приём, который отлично работает на земле. На
       доске он провалился, и по понятной причине: у земли повтор 7 × 21 на
       весь пол, у стены — 6 × 2 на панель. Пятно, нарисованное внутри плитки,
       возвращается вместе с ней, и на скриншоте стена покрылась правильной
       решёткой тёмных овалов, как плесенью. Крупная вариация на дереве обязана
       браться из РАЗНЫХ материалов (см. PROP_mt: пояса стен, ярусы настилов),
       а не из рисунка одной плитки. */
  },
  ...
  /* Те же доски, но ПОПЕРЁК: стены зала «Лесопилки» обшиты вертикальной
     ...
     ЗЕРНО ДРУГОЕ. Раньше vplank был буквально plank, повёрнутый на 90°, — то
     есть стена и настил несли ОДИН И ТОТ ЖЕ рисунок с точностью до поворота.
     В кадре, где стена и настил сходятся под прямым углом (а это половина
     ракурсов зала), совпадение видно и читается как «всё одинаковое». */
  vplank(g,S){
    g.save();
    g.translate(S, 0); g.rotate(Math.PI/2);
    TX_GEN.plank(g, S, 4409);
    g.restore();
  },
  ...
  /* КИРПИЧ. Добавлен по жалобе «всё дерево на одно лицо»: в наборе не было ни
     одной кладки, и любая попытка разбавить зал упиралась в то, что кроме
     доски, бруса и листа железа положить было нечего.

     Три вещи делают кладку кладкой, и все три здесь есть:
       * ПЕРЕВЯЗКА. Ряды смещены на полкирпича, и смещение чередуется — без
         этого получается сетка, а не стена.
       * РАЗБРОС ПО ЦВЕТУ МЕЖДУ КИРПИЧАМИ. Обожжённый кирпич никогда не бывает
         однотонным: часть уходит в тёмно-бурый, часть в песочный. Разброс тут
         ЗНАЧИТЕЛЬНО больше, чем у доски, и это можно себе позволить — у кладки
         нет вытянутого волокна, вдоль которого полосатость бросается в глаза.
       * ШОВ. Раствор светлее кирпича и утоплен: сверху тень, снизу блик.
     Плюс выщербины и потёки — ими кладка отличается от плитки в ванной. */
  brick(g,S){
    const ink = TX_ink(1.0, 0.90, 0.82);
    const u = S/256;
    TX_srand(9151);
    g.fillStyle = ink(0.62); g.fillRect(0,0,S,S);          // раствор
    const rows = 8, hh = S/rows, bw = S/4;                  // 4 кирпича в ряду
    for(let r=0;r<rows;r++){
      const y0 = r*hh, off = (r & 1) ? bw*0.5 : 0;
      for(let c=-1;c<4;c++){
        const x0 = c*bw + off;
        const v = TX_rr(0.72, 1.12);
        const wr = TX_rr(0.94, 1.07), wg = TX_rr(0.95, 1.05);
        const bi = TX_ink(1.0*wr, 0.90*wg, 0.82*TX_rr(0.92,1.08));
        const mx = 1.6*u, my = 1.4*u;                        // толщина шва
        g.fillStyle = bi(v);
        g.fillRect(x0+mx, y0+my, bw-mx*2, hh-my*2);
        // фаска: верх светлее, низ темнее — кирпич получает объём
        g.fillStyle = bi(v*1.14, 0.55); g.fillRect(x0+mx, y0+my, bw-mx*2, 1.6*u);
        g.fillStyle = bi(v*0.72, 0.55); g.fillRect(x0+mx, y0+hh-my-1.8*u, bw-mx*2, 1.8*u);
        // выщербины по кромке — у каждого пятого
        if(TX_r() < 0.22){
          const px = x0 + TX_rr(4,bw-4)*u/u, py = y0 + (TX_r()<0.5 ? my+2*u : hh-my-2*u);
          TX_wrap(S, px, py, 5*u, (qx,qy)=>{
            g.fillStyle = bi(v*0.62, 0.8); TX_blob(g,qx,qy,TX_rr(2,4.5)*u,0.5,7); g.fill(); });
        }
        // редкий пережжённый кирпич — тёмное пятно в кладке
        if(TX_r() < 0.10){ g.fillStyle = bi(0.52, 0.65); g.fillRect(x0+mx, y0+my, bw-mx*2, hh-my*2); }
      }
    }
    // высолы и подтёки поверх всей кладки
    const salt = TX_ink(0.98, 0.99, 1.0);
    for(let i=0;i<5;i++)
      TX_soft(g,S,salt, TX_rr(0,S), TX_rr(0,S), TX_rr(0.10,0.24)*S, TX_rr(1.06,1.20), TX_rr(0.4,0.7), TX_rr(0.35,0.9), TX_rr(0,3.14));
    for(let i=0;i<20;i++){
      const l = TX_rr(0.15,0.5)*S;
      TX_hair(g,S,TX_rr(0,S),TX_rr(0,S), TX_rr(-4,4)*u, l, TX_rr(2,5)*u, ink(TX_rr(0.72,0.92), 0.22));
    }
    TX_speck(g,S,ink,180,0.5*u,1.8*u,0.62,1.0,0.35);
  },
  ...
  /* ПЕРЕРИСОВАНО ВМЕСТЕ С ДОСКОЙ: заказчик просил «текстуру ЗЕМЛИ и дерева
     сделать интереснее». Пол зала — 44 × 134 м одной картой, и это самая
     большая поверхность в игре; ровная крапчатая заливка на ней читается как
     линолеум. Добавлено то, чем настоящий земляной пол цеха и отличается:
     утоптанные до глины проплешины, россыпь щепы и опилок, тёмные пятна масла
     под станинами и глубокие колеи от тележек. Всё крупное — на 100 м
     собирается в ровный тон, как требует §3.2. */
  dirt(g,S){
    const ink = TX_ink(1.0, 0.95, 0.875);
    const u = S/256;
    TX_srand(1721);
    g.fillStyle = ink(0.74); g.fillRect(0,0,S,S);
    TX_mottle(g,S,ink,16,16*u,40*u,0.60,1.02,0.40);
    // утоптанная глина: светлые плешины другого оттенка, а не той же краски
    const clay = TX_ink(1.02, 0.955, 0.855);
    for(let i=0;i<6;i++)
      TX_soft(g,S,clay, TX_rr(0,S), TX_rr(0,S), TX_rr(0.13,0.30)*S, TX_rr(1.02,1.16), TX_rr(0.5,0.8), TX_rr(0.45,1.0), TX_rr(0,3.14));
    for(let i=0;i<26;i++) TX_pebble(g,S,ink, TX_rr(0,S), TX_rr(0,S), TX_rr(2,6)*u, TX_rr(0.7,0.98));
    for(let i=0;i<5;i++) TX_crack(g,S,ink, TX_rr(0,S), TX_rr(0,S), TX_rr(24,60)*u, 0.50);
    // борозды от колёс и ног
    for(let i=0;i<16;i++){
      const a = TX_rr(-0.35,0.35), l = TX_rr(30,80)*u;
      TX_hair(g,S,TX_rr(0,S),TX_rr(0,S), Math.cos(a)*l, Math.sin(a)*l, TX_rr(3,8)*u, ink(TX_rr(0.62,0.80), 0.28));
    }
    // колея тележки: две параллельные тёмные полосы, вдавленные в грунт
    for(let i=0;i<3;i++){
      const a = TX_rr(0,3.14), cx = TX_rr(0,S), cy = TX_rr(0,S), l = TX_rr(0.5,0.95)*S;
      const nx = -Math.sin(a)*7*u, ny = Math.cos(a)*7*u;
      for(const s of [-1,1])
        TX_hair(g,S,cx+nx*s,cy+ny*s, Math.cos(a)*l, Math.sin(a)*l, TX_rr(5,9)*u, ink(TX_rr(0.50,0.66), 0.34));
    }
    // щепа и опилки — светлые продолговатые крошки, порода дерева на земле
    const chip = TX_ink(1.0, 0.93, 0.79);
    for(let i=0;i<70;i++){
      const a = TX_rr(0,3.14), l = TX_rr(3,11)*u;
      TX_hair(g,S,TX_rr(0,S),TX_rr(0,S), Math.cos(a)*l, Math.sin(a)*l, TX_rr(1.2,2.6)*u, chip(TX_rr(1.10,1.30), TX_rr(0.35,0.7)));
    }
    /* Масляные пятна под станинами — единственный не-земляной тон на полу.
       Их два и они бледные: любой рисунок внутри плитки при повторе 7 × 21
       возвращается каждые шесть метров, и чёрное круглое пятно в такой решётке
       читается сразу. Первая проба ставила четыре тёмных — на скриншоте пол
       покрылся россыпью одинаковых клякс. Масло должно угадываться. */
    const oil = TX_ink(0.86, 0.90, 0.98);
    for(let i=0;i<2;i++)
      TX_soft(g,S,oil, TX_rr(0,S), TX_rr(0,S), TX_rr(0.05,0.10)*S, TX_rr(0.72,0.88), TX_rr(0.35,0.6), TX_rr(0.4,1.0), TX_rr(0,3.14));
    TX_speck(g,S,ink,300,0.5*u,1.8*u,0.58,0.94,0.3);
    /* КРУПНЫХ МЯГКИХ ПЯТЕН ЗДЕСЬ БОЛЬШЕ НЕТ — та же ошибка, что была на доске,
       и здесь она стоила дороже всего. Пятна радиусом до половины плитки при
       повторе 7 × 21 на пол 44 × 134 м возвращаются каждые шесть метров, и на
       скриншоте пол читался леопардовым: тёмные кляксы правильной решёткой по
       всему залу. Сперва их приняли за тень от кровли — крышу даже заставили
       тень бросать (45_map.js), — но узор остался, потому что он нарисован
       прямо в плитке.
       Крупная неоднородность пола теперь берётся из РАЗНЫХ поверхностей:
       дорожки вдоль створов, круги опилок у пильных рам и у ворот, бетон базы
       (SAW_buildShell и SAW_dressBase). Внутри плитки — только та деталь,
       которая мельче метра и потому не образует видимого периода. */
  }
};
...
const TEX = {
  ...
  list: ['plank','vplank','wood','metal','plate','rust','conc','stone','sand','cloth','roof','crate','brick',
         'grate','rune','runeglow','runewood','runewoodglow','grass','dirt'],

  /* РАЗРЕШЕНИЕ ПО ПОРОДЕ. Раньше все карты рисовались в 256 — «потолок по
     контракту». Потолок этот выведен из размера в памяти, но платят за него не
     все одинаково: доска настила лежит с повтором 3×8 на пролёт в 44 м, то
     есть одна плитка накрывает больше трёх метров, и вблизи от неё остаётся
     мыло. Заказчик это и называет «некачественно».
     Крупным породам дерева и земли отдано 512, остальным оставлено 256.
     Цена — восемь карт по мегабайту вместо четверти: на глаз по времени
     загрузки не заметно, в VRAM это ничто рядом с картой теней 2048². */
  /* ПРАВКА ПО ЖАЛОБЕ «текстуры мыльные вблизи». Крупным породам отдано 1024,
     а не 512: у настила повтор 3 × 8 на пролёт 44 × 134 м, то есть одна плитка
     накрывает 14 × 17 метров, и при 512 на метр приходится 35 текселей — вдвое
     меньше, чем нужно, чтобы вблизи читалась доска, а не пятно. При 1024 это
     70 текселей на метр, и рисунок держится с двух шагов.
     Цена: восемь карт по 4 МБ вместо 1 МБ. В VRAM это меньше, чем одна карта
     теней 2048², а генерация укладывается в те же доли секунды при загрузке. */
  big: { plank:2, vplank:2, wood:2, dirt:2, crate:1, roof:1, stone:2, conc:1, brick:2,
         metal:1, plate:1, rust:1, grate:1 },
  sizeOf(name){ const k = this.big[name]; return k ? this.size*(k===2?4:2) : this.size; },

  canvas(name){
    let c = this._cv.get(name);
    if(c) return c;
    const S = this.sizeOf(name);
  ...
  get(name, rx, ry){
    ...
    /* Анизотропия поднята с 4 до предела карты (обычно 16). Она и решает
       «мыльности вблизи» на ПОЛУ и НАСТИЛАХ: их видно под острым углом, где
       обычная мип-фильтрация выбирает уровень по худшей оси и размазывает
       рисунок вдоль взгляда. Стоит это выборок только на тех пикселях, где
       угол действительно острый. */
